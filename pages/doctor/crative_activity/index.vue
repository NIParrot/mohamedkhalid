<template>
  <div class="row">
                <div data-link="/doctor/crative_activity/add" class="btn text-right ml-auto" onclick="$nuxt.$router.push(this.getAttribute('data-link'))">
                 اضافه نظام علاجي
            </div>

  <table id="crative_table" class="display mt-5 pa" style="width: 100%">
<thead>
        <tr>
          <th>الاسم</th>
          <th>صوره</th>
        ...
        columnDefs: [
          {
            ...
            className: "d-none",
        				...
        				ajax: $nuxt.$axios.defaults.baseURL + '/artical/index?type=crative_activity',
        columns: [
          { data: "name",
      render: function(data, type, row, meta) {
              return `
              <div data-link="/doctor/crative_activity/${ row.id }" style="cursor: pointer; color:#0d6efd; width:100% " class="btn btn-primary" onclick="$nuxt.$router.push(this.getAttribute('data-link'))">
                  ${ data}
            </div>`
            },
          },
  ...
  " class="delete_button" data-toggle="modal" data-target="#deleteElement">
    ...
    `,
          },
          {
            ...
            render: function(data, type, row, meta) {
                          return `
              <div data-link="/doctor/crative_activity/edit/${data}/"
              style="
                  padding: 0.5rem 1rem;
                  border-radius: 5px;
                  left: 1rem;
