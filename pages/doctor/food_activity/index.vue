<template>
  <div class="row">
                <div data-link="/doctor/food_activity/add" class="btn text-right ml-auto" onclick="$nuxt.$router.push(this.getAttribute('data-link'))">
                 اضافه نظام غذائي
            </div>
...
export default {
  ...
  methods: {
			...
			$('body').on(
				'click',
				'#food_table tbody tr .delete_button',
				function () {
					select = {
						row: $('#food_table').DataTable().row(this.parentNode),
						id: $('#food_table')
							.DataTable()
							.row(this.parentNode)
							.data().id,
        ...
        columnDefs: [
          {
            ...
            className: "d-none",
        				...
        				ajax: $nuxt.$axios.defaults.baseURL + '/artical/index?type=food_activity',
        columns: [
          { data: "name",
  ...
  " class="delete_button" data-toggle="modal" data-target="#deleteElement">
    ...
    `,
          },
          {
            ...
            render: function(data, type, row, meta) {
                          return `
              <div data-link="/doctor/food_activity/edit/${data}/"
              style="
                  padding: 0.5rem 1rem;
                  border-radius: 5px;
                  left: 1rem;
