<template>
  <div class="row">
                <nuxt-link class="btn text-right ml-auto" to="/dashboard/doctor/add">
                 اضافه دكتور
                </nuxt-link>
    <table id="doctor_table" class="display mt-5 pa" style="width: 100%">
      <thead>
        <tr>
          <th>اسم المستخدم</th>
          <th>الاسم</th>
...
export default {
  ...
  methods: {
			...
			$('body').on(
				'click',
				'#doctor_table tbody tr .delete_button',
				function () {
					select = {
						row: $('#doctor_table').DataTable().row(this.parentNode),
						id: $('#doctor_table')
							.DataTable()
							.row(this.parentNode)
							.data().id,
					...
					$('.confirm-delete').click(function () {
						$nuxt.$store
							.dispatch('doctor/deleteDoctor', { id: select.id })
							.then((doctor) => {
          ...
          fetch({ $axios, store }) {
    return $axios.$get("/doctor/add").then(res => {
      store.commit("doctor/updatedoctors", res);
    });
  },
        updateSelecteddoctor(doctor) {
      this.$store.commit("updateSelecteddoctor", doctor);
    },
    onScriptLoaded() {
      this.externalLoaded = true
      console.log('script loaded')
      this.dataTable = $('#doctor_table').DataTable({
        language: {
        ...
        columnDefs: [
          {
            ...
            className: 'd-none',
				...
				ajax: $nuxt.$axios.defaults.baseURL + '/doctor/index',
        columns: [
          {
            data: 'username',
            render: function (data, type, row, meta) {
            return `
              <div data-link="/Dashboard/doctor/${ row.id }" style="cursor: pointer; color:#0d6efd; width:100% " class="btn btn-primary" onclick="$nuxt.$router.push(this.getAttribute('data-link'))">
                  ${ data}
            </div>`
            },
          },
  ...
  " class="delete_button" data-toggle="modal" data-target="#deleteElement">
    ...
    `,
          },
          {
            ...
            render: function (data, type, row, meta) {
              return `
              <div data-link="/Dashboard/doctor/edit/${data}/"
              style="
                  padding: 0.5rem 1rem;
                  border-radius: 5px;
                  left: 1rem;
                ...
                "class="edit_button"
              onclick="
              $nuxt.$store.commit('doctor/updateSelecteddoctor', this.row);
