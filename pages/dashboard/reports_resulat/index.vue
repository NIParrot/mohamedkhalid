<template>
  <div class="row">
                <div data-link="/Dashboard/reports_resulat/add" class="btn text-right ml-auto" onclick="$nuxt.$router.push(this.getAttribute('data-link'))">
                 اضافه نتيجه القرار
            </div>
  <table id="faculity_table" class="display mt-5 pa" style="width: 100%">
<thead>
        <tr>
          <th>الاسم</th>
          <th>الوصف</th>
...
language: {
    ...
    "aria": {
        ...
        ajax: "https://app.infratechtechnology.org/api/admin/worktime",
        columns: [
          {
            data: 'start_time',
            render: function (data, type, row, meta) {
            return `
              <div data-link="/Dashboard/reports_resulat/${ row.id }" style="cursor: pointer; color:#0d6efd; width:100% " class="btn btn-primary" onclick="$nuxt.$router.push(this.getAttribute('data-link'))">
                  ${ data}
            </div>`
            },
          },
  ...
  " class="delete_button" data-toggle="modal" data-target="#deleteElement">
    ...
    `,
          },
          {
            ...
            render: function(data, type, row, meta) {
              return `
              <div data-link="/Dashboard/reports_resulat/edit/${data}/"
              style="
                  padding: 0.5rem 1rem;
                  border-radius: 5px;
                  left: 1rem;
