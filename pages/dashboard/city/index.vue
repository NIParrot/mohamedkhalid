<template>
  <div class="row">
                <nuxt-link class="btn text-right ml-auto" to="/dashboard/city/add">
                 اضافه مدينه
                </nuxt-link>
    <table id="city_table" class="display mt-5 pa" style="width: 100%">
      <thead>
        <tr>
          <th>الاسم</th>
          <th>المحافظه</th>
...
export default {
  ...
  methods: {
    ...
    return $axios.$get("/city/add").then(res => {
      ...
      this.$store.commit("updateSelectedcity", city);
    },
    onScriptLoaded() {
      this.externalLoaded = true
      console.log('script loaded')
      this.dataTable = $('#city_table').DataTable({
        language: {
        ...
        columnDefs: [
          {
            ...
            className: 'd-none',
				...
				ajax: $nuxt.$axios.defaults.baseURL + '/city/index',
        columns: [
          {
            data: 'name',
            render: function (data, type, row, meta) {
            return `
              <div data-link="/Dashboard/city/${ row.id }" style="cursor: pointer; color:#0d6efd; width:100% " class="btn btn-primary" onclick="$nuxt.$router.push(this.getAttribute('data-link'))">
                  ${ data}
            </div>`
            },
          },
  ...
  " class="delete_button" data-toggle="modal" data-target="#deleteElement">
    ...
    `,
          },
          {
            ...
            render: function (data, type, row, meta) {
              return `
              <div data-link="/Dashboard/city/edit/${data}"
              style="
                  padding: 0.5rem 1rem;
                  border-radius: 5px;
                  left: 1rem;
