  ...
  methods: {
    ...
    onScriptLoaded() {
      ...
      this.dataTable = $('#user_table').DataTable({
        ...
        ajax:  $nuxt.$axios.defaults.baseURL + '/patient/index',
        columns: [
          {
            data: 'fullname',
            render: function (data, type, row, meta) {
            return `
              <div data-link="/dashboard/user/${ row.id }" style="cursor: pointer; color:#0d6efd; " class="btn btn-primary" onclick="$nuxt.$router.push(this.getAttribute('data-link'))">
                  ${ data}
            </div>`
            },
          },
